...
def manuscript_text(results: dict, word_count: int = 0) -> list[tuple[str, str]]:
    ...
    return [
        ...
        ("normal", "The immediate problem at admission extends beyond prognostic labeling. It includes monitored beds, nurse attention, repeat assessment, and senior review while diagnostic and treatment pathways are still moving. Although a score can perform well as a population predictor, bedside monitoring decisions require an operating point chosen for triage safety. In a referral center, the clinical cost of missing early deterioration is high, but the capacity cost of excessive escalation is also real."),
        ...
        ("normal", "We evaluated a triage model using a random forest, an ensemble of tree predictors [6]. The model used 12 pre-specified routine admission variables. The aim was not to replace GRACE or clinician judgment. Instead, the study tested an internally validated referral-center monitoring aid with a priori operating rules. The objective was to estimate discrimination, calibration, and clinical tier performance for an admission-time model, compare its AUC with GRACE 2.0 on the same cohort, and quantify how a post-admission cardiogenic shock variable would inflate apparent performance."),
        ("heading", "Methods"),
        ...
        ("normal", f"Calibration was close to the ideal line. The calibration slope was {cal['slope']:.3f}, calibration-in-the-large was {cal['citl']:.3f}, O:E ratio was {cal['oe_ratio']:.3f}, and ECE was {cal['ece']:.3f}. Figure 2 shows the reliability pattern across risk bins. These estimates came from cross-validated predictions, not from the data used to develop the model."),
...
def build_manuscript(results: dict) -> int:
    ...
    doc.add_paragraph("Table 2. Model and GRACE 2.0 performance in cross-validated analysis.")
